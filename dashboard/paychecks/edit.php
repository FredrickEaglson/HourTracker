...
$sql = $con->prepare("SELECT * FROM `paychecks` WHERE `pcid`=? && `userid`=?");
                    ...
                    <form class="w-full max-w-md" method="post">
                        ...
                        <div class="grid grid-cols-3 grid-rows-2 gap-4">
                            <div class="p-2 bg-slate-200 rounded border border-black border-solid">
                                ...
                                <input type="date" class="max-w-full border border-black" name="date" value="<?php echo date("Y-m-d", strtotime($row['date'])); ?>">
                            </div>
                            <div class="p-2 bg-slate-200 rounded border border-black border-solid">
                            ...
                            <div class="col-span-3 ">
                                <div class="grid grid-cols-4 gap-2">
                                    <div class="p-2 bg-green-200 rounded border border-black border-solid">
                                        ...
                                        <input type="text" class="max-w-full" name="hourly" readonly value="<?php echo $formatter->formatCurrency($row['hourly'], "USD"); ?>">
                                    </div>
                                    <div class="p-2 bg-green-200 rounded border border-black border-solid">
                                        <label for="periodID">Tips</label>
                                        <input type="text" class="max-w-full" name="tips" value="<?php echo $formatter->formatCurrency($row['tips'], "USD"); ?>">
                                    </div>

                                    <div class="p-2 bg-green-200 rounded border border-black border-solid">
                                        <label for="periodID">Taxes</label>
                                        <input type="text" class="max-w-full" name="taxes" value="<?php echo $formatter->formatCurrency($row['taxes'], "USD"); ?>">
                                    </div>
                                    <div class="p-2 bg-green-200 rounded border border-black border-solid">
                                        <label for="periodID">Deductions</label>
                                        <input type="text" class="max-w-full" name="deductions" value="<?php echo $formatter->formatCurrency($row['deductions'], "USD"); ?>">
                                    </div>
                                </div>
                            </div>

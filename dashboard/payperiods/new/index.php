...
<body class="w-screen">
        ...
        <section class="place-content-center">
            <div class="flex flex-col justify-center items-center p-3 m-4 border-solid rounded-4xl  border-4 border-black shadow-2xl">
                ...
                <div class="flex flex-col justify-center items-center w-full">
                    <form class="w-full max-w-md" action="new/index.php" method="post">
                        <div class="grid grid-cols-3 grid-rows-2 gap-4">
                            ...
                            <div class="p-2 bg-slate-200 rounded border border-black border-solid">
                                ...
                                <input type="date" class="max-w-full" name="startdate">
                            </div>
                            <div class="p-2 bg-slate-200 rounded border border-black border-solid">
                                <label for="enddate">End Date</label>
                                <input type="date" class="max-w-full" name="enddate">
                            </div>
                            <div class="p-2 bg-slate-200 rounded border border-black border-solid">
                        ...
                        <div class="flex flex-col justify-center items-center p-3 m-4">
                            <form action="javascript:;" method="POST">
                                <table class="border1 border-collapse gap-2">
                                    ...
                                    <tr>
                                        <form action="javascript:;" id="newShiftForm">
                                        <th><button id="new" type="submit">New</button></th>
                                        <td><input type="date" name="newdate"></td>
                                        <td><input class="in1" type="number" name="newhours"></td>
                                        <td><input class="in1" type="number" name="newminutes"></td>
                                        <td><input class="in1" type="number" name="newrate" step="0.01" value="<?php echo $defaultrate ?>"></td>
                                        </form>
                                    </tr>
                                    <tbody id="tbodshifts">
                                        <?php
                                        ...
                                        if ($result->num_rows == 0&&sizeof($_SESSION['tempshifts'])==0) {
    ...
    <script>
        ...
        document.getElementById("newShiftForm").addEventListener("submit", function(event) {
            event.preventDefault();

            var Xhttp = new XMLHttpRequest();
            Xhttp.open("POST", "auth/db.php", true);
            Xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            Xhttp.send("r=newshift&shiftdate=" + document.getElementById("newdate").value + "&shifthours=" + document.getElementById("newhours").value + "&shiftminutes=" + document.getElementById("newminutes").value + "&shiftrate=" + document.getElementById("newrate").value);
        });
    </script>
</body>
